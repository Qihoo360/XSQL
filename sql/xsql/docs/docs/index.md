...
| 2.3.1.xsql-0.3.2 | 2019.05.25 | 1. bin/spark-xsql支持${}字符串变量<br />2. datasource关键字与列名冲突<br />3. processSingleTable assert失败<br />4. !开头的命令cast报错<br />5. OutOfDriectMemoryError | | 1. 支持Hive的remove datasource if exists<br />2. 支持Hive的insert overwrite directory using xxx<br />3. 增加Yarn优先级控制参数spark-sql --priority |
| 2.3.1.xsql-0.4.0 | 2019.07.01 | 1. load data partition<br/>2. StackOverflow caused by window functions<br/> 3. select using script| 1. set 不再触发job <br/>2. local模式不再产生日志文件  | 1.支持Hive的distribute by<br/> 2.接入Kafka数据源 |
| 2.4.3.xsql-0.5.0 | 2019.08.01 | 1.修复任务结束时报错java.io.IOException: Filesystem closed<br />2.解决HDFS0.2版本与Spark流式SQL所使用HDFS高版本的兼容性问题<br/>3.修复MongoManager、ElasticSearchManager在Local模式下，使用SparkSession.getActiveSession的bug<br/>4.解决XSQL升级Spark2.4.3后，流式SQL无法支持下推的问题 | 升级至Spark2.4.3核心   | 支持流式SQL计算Kafka数据 |
| 2.4.3.xsql-0.6.0 | 2019.08.30 | 1.修改SparkXSQLShell中错误的打印SparkHiveShell信息 | 1.通过ServiceLoader加载DatasourceManager实现<br/>2.增加工作数据源，减少在数据字典读写过程中，反复获取数据源<br/>3.DataSourceManager继承ExternalCatalog，减少相同的方法签名<br/>4.修改mkdocs文档风格<br/>5.增加xsql.conf.template<br/>6.添加监控模块 | 1.支持插件化<br/>2.增加中英文README<br/>3.增加LOGO<br/> |
| 2.4.3.xsql-0.6.1 | 2019.08.11 | 1.[Rename the prefix of jars from spark to xsql.](https://github.com/Qihoo360/XSQL/pull/1)<br/>2.[Fix the bug that can't find default database when default database's name is not default](https://github.com/Qihoo360/XSQL/pull/21)<br/>3.[Solve sql select query that select is followed by \n can't run when we use -e sql](https://github.com/Qihoo360/XSQL/pull/22)<br/>4.[Fix cannot find spark-xsql-shell.](https://github.com/Qihoo360/XSQL/pull/35)<br/>5.[.hiverc is loaded twice making create function throw exception](https://github.com/Qihoo360/XSQL/pull/45)<br/>6.[Correct the content of license file for XSQL.](https://github.com/Qihoo360/XSQL/pull/50) | 1.[Fix com.fasterxml.jackson.core:jackson-databind vulnerabilities.](https://github.com/Qihoo360/XSQL/pull/24)<br/>2.[Fix org.jpmml:pmml-model vulnerabilities.](https://github.com/Qihoo360/XSQL/pull/25)<br/>3.[Fix zookeeper vulnerabilities](https://github.com/Qihoo360/XSQL/pull/27)<br/>4.[build-plugin.sh should use build/mvn to find mvn or install mvn](https://github.com/Qihoo360/XSQL/pull/29)<br/>5.[Fix puppycrawl vulnerabilities](https://github.com/Qihoo360/XSQL/pull/33)6.[Fix libthrift vulnerabilities.](https://github.com/Qihoo360/XSQL/pull/42)<br/>6.[Migrate some XSQL test classes to github](https://github.com/Qihoo360/XSQL/pull/43)<br/>7.[Upgrade jackson-databind to 2.9.10 and fix vulnerabilities.](https://github.com/Qihoo360/XSQL/pull/44)<br/>8.[Add Github pull request template](https://github.com/Qihoo360/XSQL/pull/46) |  |
...
| <img src="images/kafka.png" width="120" height="40" />  | 0.10+   | PRODUCTION                  | [Kafka Doc](datasources/kafka.md)            |                                   |
| <img src="images/elasticsearch.png" width="120" height="60" /> | 5.2  | PRODUCTION                  | [ElasticSearch Doc](datasources/elasticsearch.md)      | POWER BY ElasticSearch-Hadoop |
| <img src="images/druid.png" width="120" height="40" /> | 0.10.0 | PRODUCTION | [Druid Doc](datasources/druid.md) |  |
| <img src="images/mongo.png" width="120" height="35" />       | 2.3.0    | PRODUCTION                  | [MongoDB Doc](datasources/mongo.md)                      | POWER BY MongoDB  Connector |
