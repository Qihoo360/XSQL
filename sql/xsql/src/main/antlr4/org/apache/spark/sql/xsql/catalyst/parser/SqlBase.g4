...
statement
    ...
    | USE dbIdentifier                                                 #use
    | CREATE DATABASE (IF NOT EXISTS)? dbIdentifier
        (COMMENT comment=STRING)? locationSpec?
        (WITH DBPROPERTIES tablePropertyList)?                         #createDatabase
    | ALTER DATABASE dbIdentifier SET DBPROPERTIES tablePropertyList   #setDatabaseProperties
    | DROP DATABASE (IF EXISTS)? dbIdentifier (RESTRICT | CASCADE)?    #dropDatabase
    ...
    | EXPLAIN (LOGICAL | FORMATTED | EXTENDED | CODEGEN | COST)?
        statement                                                      #explain
    | SHOW TABLES ((FROM | IN) dbIdentifier)?
        (LIKE? pattern=STRING)?                                        #showTables
    | SHOW TABLE EXTENDED ((FROM | IN) db=dbIdentifier)?
        LIKE pattern=STRING partitionSpec?                             #showTable
    | SHOW DATASOURCES
        (LIKE? pattern=STRING)?                                        #showDatasources
    | (ADD | CREATE) DATASOURCE identifier tablePropertyList           #addDatasource
    | REFRESH DATASOURCE identifier                                    #refreshDatasource
    | (REMOVE | DROP) DATASOURCE (IF EXISTS)? identifier               #removeDatasource
    | SHOW DATABASES ((FROM | IN) ds=identifier)?
        (LIKE? pattern=STRING)?                                        #showDatabases
    | SHOW TBLPROPERTIES table=tableIdentifier
        ('(' key=tablePropertyKey ')')?                                #showTblProperties
    | SHOW COLUMNS (FROM | IN) tableIdentifier
        ((FROM | IN) db=dbIdentifier)?                                 #showColumns
    | SHOW PARTITIONS tableIdentifier partitionSpec?                   #showPartitions
    | SHOW identifier? FUNCTIONS
        (LIKE? (qualifiedName | pattern=STRING))?                      #showFunctions
    | SHOW CREATE TABLE tableIdentifier                                #showCreateTable
    | (DESC | DESCRIBE) FUNCTION EXTENDED? describeFuncName            #describeFunction
    | (DESC | DESCRIBE) DATABASE EXTENDED? dbIdentifier                #describeDatabase
...
strictProperty
    : key=strictPropertyKey EQ value=strictPropertyValue
    ;
strictPropertyKey
    : identifier
    | DEFAULT CHARSET
...
dbIdentifier
    : (ds=identifier '.')? db=identifier
    ;

tableIdentifier
    : ((ds=identifier '.' db=identifier | db=identifier) '.')? table=identifier
    ;

functionIdentifier
    : ((ds=identifier '.' db=identifier | db=identifier) '.')? function=identifier
    ;
...
dataType
    ...
    | identifier ('(' INTEGER_VALUE (',' INTEGER_VALUE)* ')')?(UNSIGNED)?  #primitiveDataType
    ;
...
colType
    : identifier dataType (NOT NULL)? ((DEFAULT (defaultString=STRING|NULL)) | AUTO_INCREMENT)? (PRIMARY KEY)? (COMMENT commentString=STRING)?
...
nonReserved
    : SHOW | TABLES | COLUMNS | COLUMN | PARTITIONS | FUNCTIONS | DATABASES | DATASOURCES | DATASOURCE
    | ADD
    ...
    | DIRECTORY
    | BOTH | LEADING | TRAILING | AUTO_INCREMENT | DEFAULT| KEY
    ;
