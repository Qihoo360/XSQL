...
package org.apache.spark.sql.xsql.execution
...
import org.apache.spark.sql.xsql.catalyst.parser.{
  AbstractXSQLParser,
  SqlBaseParser,
  XSQLAstBuilder
}
import org.apache.spark.sql.xsql.catalyst.parser.SqlBaseParser._
import org.apache.spark.sql.xsql.execution.command._

class XSQLSqlParser(conf: SQLConf) extends AbstractXSQLParser {
  override val astBuilder = new XSQLSqlAstBuilder(conf)
...
class XSQLSqlAstBuilder(conf: SQLConf) extends XSQLAstBuilder(conf) {
   ...
   * {{{
   *   ANALYZE TABLE [[ds_name.]db_name.]tablename
   ...
   * {{{
   *   ANALYZE TABLE [[ds_name.]db_name.]tablename COMPUTE STATISTICS FOR COLUMNS column1, column2;
   * }}}
   */
  override def visitAnalyze(ctx: AnalyzeContext): LogicalPlan = withOrigin(ctx) {
    ...
    if (ctx.identifierSeq() == null) {
      if (ctx.partitionSpec != null) {
        XSQLAnalyzePartitionCommand(
          table,
          visitPartitionSpec(ctx.partitionSpec),
          noscan = ctx.identifier != null)
      } else {
        XSQLAnalyzeTableCommand(table, noscan = ctx.identifier != null)
      }
    } else {
      if (ctx.partitionSpec != null) {
        logWarning(
      ...
      XSQLAnalyzeColumnCommand(table, visitIdentifierSeq(ctx.identifierSeq()))
    }
  }

  /**
   * Create a [[XSQLSetDatabaseCommand]] logical plan.
   */
  override def visitUse(ctx: UseContext): LogicalPlan = withOrigin(ctx) {
    XSQLSetDatabaseCommand(
      Option(ctx.dbIdentifier.ds).map(_.getText),
      ctx.dbIdentifier.db.getText)
  }

  /**
   * Create a [[XSQLShowTablesCommand]] logical plan.
   * Example SQL :
   * {{{
   *   SHOW TABLES [(IN|FROM) [ds_name.]database_name] [[LIKE] 'identifier_with_wildcards'];
   * }}}
   */
  override def visitShowTables(ctx: ShowTablesContext): LogicalPlan = withOrigin(ctx) {
    val pair = Option(ctx.dbIdentifier).map(visitDbIdentifier).getOrElse((None, None))
   ...
   * Example SQL :
   * {{{
   *   SHOW TABLE EXTENDED [(IN|FROM) [ds_name.]database_name] LIKE 'identifier_with_wildcards'
  ...
  override def visitShowTable(ctx: ShowTableContext): LogicalPlan = withOrigin(ctx) {
    ...
    val pair = Option(ctx.db).map(visitDbIdentifier).getOrElse((None, None))
    XSQLShowTablesCommand(
      pair._1,
      pair._2,
      Option(ctx.pattern).map(string),
  ...
  /**
   * Create a [[XSQLShowDatasourcesCommand]] logical plan.
   * Example SQL:
   * {{{
   *   SHOW (DATABASES|SCHEMAS) [(IN|FROM) datasource_name] [LIKE 'identifier_with_wildcards'];
   * }}}
   */
  override def visitShowDatasources(ctx: ShowDatasourcesContext): LogicalPlan = withOrigin(ctx) {
    XSQLShowDatasourcesCommand(Option(ctx.pattern).map(string))
  }

  /**
   * Create a [[XSQLAddDatasourceCommand]] logical plan
   */
  override def visitAddDatasource(ctx: AddDatasourceContext): LogicalPlan = withOrigin(ctx) {
    ...
    XSQLAddDatasourceCommand(ctx.identifier().getText, properties.updated(TEMP_FLAG, "true"))
  }

  /**
   * Create a [[XSQLRemoveDatasourceCommand]] logical plan
   */
  override def visitRemoveDatasource(ctx: RemoveDatasourceContext): LogicalPlan = {
    XSQLRemoveDatasourceCommand(ctx.identifier().getText, ctx.EXISTS != null)
  }

  /**
   * Create a [[XSQLRefreshDatasourceCommand]] logical plan
   */
  override def visitRefreshDatasource(ctx: RefreshDatasourceContext): LogicalPlan = {
    XSQLRefreshDatasourceCommand(ctx.identifier().getText)
  }

  /**
   * Create a [[XSQLShowDatabasesCommand]] logical plan.
   * Example SQL:
   * {{{
   *   SHOW (DATABASES|SCHEMAS) [(IN|FROM) datasource_name] [LIKE 'identifier_with_wildcards'];
   * }}}
  ...
  override def visitShowDatabases(ctx: ShowDatabasesContext): LogicalPlan = withOrigin(ctx) {
    XSQLShowDatabasesCommand(Option(ctx.ds).map(_.getText), Option(ctx.pattern).map(string))
  }
   ...
   * }}}
   */
  override def visitShowTblProperties(ctx: ShowTblPropertiesContext): LogicalPlan =
    withOrigin(ctx) {
   ...
   * This function creates a [[XSQLShowColumnsCommand]] logical plan.
   *
   * The syntax of using this command in SQL is:
   * {{{
  ...
  override def visitShowColumns(ctx: ShowColumnsContext): LogicalPlan = withOrigin(ctx) {
    if (Option(ctx.dbIdentifier) == None) {
      XSQLShowColumnsCommand(None, None, visitTableIdentifier(ctx.tableIdentifier))
    } else {
      val pair = visitDbIdentifier(ctx.dbIdentifier)
      XSQLShowColumnsCommand(pair._1, pair._2, visitTableIdentifier(ctx.tableIdentifier))
    }
  }
   ...
   *
   * This function creates a [[XSQLShowPartitionsCommand]] logical plan
   *
   * The syntax of using this command in SQL is:
   * {{{
  ...
  override def visitShowPartitions(ctx: ShowPartitionsContext): LogicalPlan = withOrigin(ctx) {
    ...
    XSQLShowPartitionsCommand(table, partitionKeys)
  }
  ...
  /**
   * Create a [[DescribeColumnCommand]] or [[XSQLDescribeTableCommand]] logical commands.
   */
   ...
   */
  override def visitCreateTableHeader(ctx: CreateTableHeaderContext): TableHeader =
    withOrigin(ctx) {
   ...
   * Expected format:
   * {{{
   *   CREATE [TEMPORARY] TABLE [IF NOT EXISTS] [[ds_name.]db_name.]table_name
  ...
  override def visitCreateTable(ctx: CreateTableContext): LogicalPlan = withOrigin(ctx) {
    ...
    // INLINE: similiar to visitTablePropertyList
    val strictProperties = Option(ctx.strictProperty)
      .map { properties =>
        val propertiesBuffer = properties.asScala.map { property =>
          val key = visitStrictPropertyKey(property.key)
          val value = visitStrictPropertyValue(property.value)
          key -> value
        }
        checkDuplicateKeys(propertiesBuffer, ctx)
        propertiesBuffer.toMap
      }
      .getOrElse(Map.empty)
    ...
    val tableDesc = CatalogTable(
      ...
      properties = properties ++ strictProperties,
    ...
    if (ctx.query != null) {
      ...
      if (temp) {
        ...
        logWarning(
          s"CREATE TEMPORARY TABLE ... USING ... is deprecated, please use " +
  ...
  /**
   * Create a [[XSQLCreateDatabaseCommand]] command.
   *
   * For example:
   * {{{
   *   CREATE DATABASE [IF NOT EXISTS] [ds_name.]database_name [COMMENT database_comment]
   ...
   * }}}
   */
  override def visitSetDatabaseProperties(ctx: SetDatabasePropertiesContext): LogicalPlan =
    withOrigin(ctx) {
      XSQLAlterDatabasePropertiesCommand(
        Option(ctx.dbIdentifier.ds).map(_.getText),
        ctx.dbIdentifier.db.getText,
  ...
  /**
   * Create a [[XSQLDropDatabaseCommand]] command.
   *
   * For example:
   * {{{
  ...
  /**
   * Create a [[XSQLDescribeDatabaseCommand]] command.
   *
   * For example:
   * {{{
   ...
   * }}}
   */
  override def visitDescribeDatabase(ctx: DescribeDatabaseContext): LogicalPlan =
    withOrigin(ctx) {
      XSQLDescribeDatabaseCommand(
        Option(ctx.dbIdentifier.ds).map(_.getText),
        ctx.dbIdentifier.db.getText,
        ctx.EXTENDED != null)
    }
  ...
  /**
   * Create a [[XSQLTruncateTableCommand]] command.
   *
   * For example:
   * {{{
  ...
  /**
   * Create an [[XSQLAlterTableSetPropertiesCommand]] command.
   *
   * For example:
   * {{{
   ...
   * }}}
   */
  override def visitSetTableProperties(ctx: SetTablePropertiesContext): LogicalPlan =
    withOrigin(ctx) {
      XSQLAlterTableSetPropertiesCommand(
        visitTableIdentifier(ctx.tableIdentifier),
        visitPropertyKeyValues(ctx.tablePropertyList),
        ctx.VIEW != null)
    }
  ...
  /**
   * Create an [[XSQLAlterTableUnsetPropertiesCommand]] command.
   *
   * For example:
   * {{{
   ...
   * }}}
   */
  override def visitUnsetTableProperties(ctx: UnsetTablePropertiesContext): LogicalPlan =
    withOrigin(ctx) {
      XSQLAlterTableUnsetPropertiesCommand(
        visitTableIdentifier(ctx.tableIdentifier),
  ...
  /**
   * Create an [[XSQLAlterTableSerDePropertiesCommand]] command.
   *
   * For example:
   * {{{
  ...
  override def visitSetTableSerDe(ctx: SetTableSerDeContext): LogicalPlan = withOrigin(ctx) {
    XSQLAlterTableSerDePropertiesCommand(
      visitTableIdentifier(ctx.tableIdentifier),
   ...
   */
  override def visitAddTablePartition(ctx: AddTablePartitionContext): LogicalPlan =
    withOrigin(ctx) {
      if (ctx.VIEW != null) {
      ...
      val specsAndLocs = if (ctx.partitionSpec.isEmpty) {
        ctx.partitionSpecLocation.asScala.map { splCtx =>
   ...
   * }}}
   */
  override def visitRenameTablePartition(ctx: RenameTablePartitionContext): LogicalPlan =
    withOrigin(ctx) {
   ...
   * is associated with physical tables
   */
  override def visitDropTablePartitions(ctx: DropTablePartitionsContext): LogicalPlan =
    withOrigin(ctx) {
      if (ctx.VIEW != null) {
   ...
   * }}}
   */
  override def visitRecoverPartitions(ctx: RecoverPartitionsContext): LogicalPlan =
    withOrigin(ctx) {
   ...
   * }}}
   */
  override def visitSetTableLocation(ctx: SetTableLocationContext): LogicalPlan =
    withOrigin(ctx) {
  ...
  /**
   * Create a [[XSQLAlterTableAddColumnsCommand]] command.
   *
   * For example:
   * {{{
   *   ALTER TABLE table1
  ...
  override def visitAddTableColumns(ctx: AddTableColumnsContext): LogicalPlan = withOrigin(ctx) {
    XSQLAlterTableAddColumnsCommand(
      visitTableIdentifier(ctx.tableIdentifier),
  ...
  /**
   * Create a [[XSQLAlterTableChangeColumnCommand]] command.
   *
   * For example:
   * {{{
  ...
  override def visitChangeColumn(ctx: ChangeColumnContext): LogicalPlan = withOrigin(ctx) {
    ...
    XSQLAlterTableChangeColumnCommand(
      visitTableIdentifier(ctx.tableIdentifier),
  ...
  override def visitStrictPropertyKey(key: StrictPropertyKeyContext): String = {
    if (key.DEFAULT != null && key.CHARSET() != null) {
      "DEFAULT CHARSET"
    } else {
      key.getText
    }
  ...
  override def visitStrictPropertyValue(value: StrictPropertyValueContext): String = {
    if (value.tablePropertyValue() != null) {
      visitTablePropertyValue(value.tablePropertyValue)
    } else {
      value.getText
    }
  }
   ...
   * Expected format:
   * {{{
   *   CREATE [EXTERNAL] TABLE [IF NOT EXISTS] [[ds_name.]db_name.]table_name
  ...
  override def visitCreateHiveTable(ctx: CreateHiveTableContext): LogicalPlan = withOrigin(ctx) {
    ...
    if (temp) {
      throw new ParseException(
        "CREATE TEMPORARY TABLE is not supported yet. " +
          "Please use CREATE TEMPORARY VIEW as an alternative.",
        ctx)
    }
    ...
    val fileStorage = ctx.createFileFormat.asScala.headOption
      .map(visitCreateFileFormat)
      .getOrElse(CatalogStorageFormat.empty)
    val rowStorage = ctx.rowFormat.asScala.headOption
      .map(visitRowFormat)
      .getOrElse(CatalogStorageFormat.empty)
    val location = ctx.locationSpec.asScala.headOption.map(visitLocationSpec)
   ...
   * For example:
   * {{{
   *   CREATE TABLE [IF NOT EXISTS] [[ds_name.]db_name.]table_name
   ...
   * Format: STORED AS ...
   */
  override def visitCreateFileFormat(ctx: CreateFileFormatContext): CatalogStorageFormat =
    withOrigin(ctx) {
   ...
   */
  override def visitTableFileFormat(ctx: TableFileFormatContext): CatalogStorageFormat =
    withOrigin(ctx) {
      CatalogStorageFormat.empty
        .copy(inputFormat = Option(string(ctx.inFmt)), outputFormat = Option(string(ctx.outFmt)))
    }

  /**
   * Resolve a [[HiveSerDe]] based on the name given and return it as a [[CatalogStorageFormat]].
   */
  override def visitGenericFileFormat(ctx: GenericFileFormatContext): CatalogStorageFormat =
    withOrigin(ctx) {
   ...
   */
  override def visitRowFormatSerde(ctx: RowFormatSerdeContext): CatalogStorageFormat =
    withOrigin(ctx) {
      import ctx._
      CatalogStorageFormat.empty.copy(
   ...
   */
  override def visitRowFormatDelimited(ctx: RowFormatDelimitedContext): CatalogStorageFormat =
    withOrigin(ctx) {
   ...
   * For example:
   * {{{
   *   CREATE [OR REPLACE] [[GLOBAL] TEMPORARY] VIEW [IF NOT EXISTS] [[ds_name.]db_name.]view_name
   ...
   * For example:
   * {{{
   *   ALTER VIEW [[ds_name.]db_name.]view_name AS SELECT ...;
   * }}}
   */
   ...
   */
  override def visitCreateTempViewUsing(ctx: CreateTempViewUsingContext): LogicalPlan =
    withOrigin(ctx) {
      CreateTempViewUsing(
   ...
   * For example:
   * {{{
   *   CREATE [OR REPLACE] [TEMPORARY] FUNCTION [IF NOT EXISTS] [[ds_name.]db_name.]function_name
  ...
  override def visitCreateFunction(ctx: CreateFunctionContext): LogicalPlan = withOrigin(ctx) {
    ...
    XSQLCreateFunctionCommand(
      functionIdentifier.dataSource,
  ...
  override def visitShowFunctions(ctx: ShowFunctionsContext): LogicalPlan = withOrigin(ctx) {
    ...
    val (ds, db, pat) = if (qualifiedName != null) {
      val name = visitFunctionName(qualifiedName)
      (name.dataSource, name.database, Some(name.funcName))
    } else if (pattern != null) {
      (None, None, Some(string(pattern)))
    } else {
      (None, None, None)
    }

    XSQLShowFunctionsCommand(ds, db, pat, user, system)
  }
   ...
   */
  override def visitDescribeFunction(ctx: DescribeFunctionContext): LogicalPlan =
    withOrigin(ctx) {
      import ctx._
      val functionName =
  ...
  override def visitCacheTable(ctx: CacheTableContext): LogicalPlan = withOrigin(ctx) {
    ...
    if (query.isDefined && tableIdent.database.isDefined) {
      ...
      throw new ParseException(
        s"It is not allowed to add database prefix `$database` to " +
          s"the table name in CACHE TABLE AS SELECT",
        ctx)
    }
   ...
   */
  override def visitSetConfiguration(ctx: SetConfigurationContext): LogicalPlan =
    withOrigin(ctx) {
  ...
  override protected def withScriptIOSchema(
    ...
    def format(fmt: RowFormatContext, configKey: String, defaultConfigValue: String): Format =
      fmt match {
        case c: RowFormatDelimitedContext =>
          // we should use the visitRowFormatDelimited function here. However HiveScriptIOSchema
        ...
        case c: RowFormatSerdeContext =>
          // Use a serde format.
          val CatalogStorageFormat(None, None, None, Some(name), _, props) = visitRowFormatSerde(
            c)
   ...
   * }}}
   */
  override def visitInsertOverwriteHiveDir(ctx: InsertOverwriteHiveDirContext): InsertDirParams =
    withOrigin(ctx) {
      validateRowFormatFileFormat(ctx.rowFormat, ctx.createFileFormat, ctx)
      val rowStorage = Option(ctx.rowFormat)
        .map(visitRowFormat)
        .getOrElse(CatalogStorageFormat.empty)
      val fileStorage = Option(ctx.createFileFormat)
        .map(visitCreateFileFormat)
        .getOrElse(CatalogStorageFormat.empty)
