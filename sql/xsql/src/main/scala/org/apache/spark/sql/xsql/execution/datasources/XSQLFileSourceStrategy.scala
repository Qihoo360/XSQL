 ...
 */
object XSQLFileSourceStrategy extends Strategy with Logging {
  def apply(plan: LogicalPlan): Seq[SparkPlan] = plan match {
    case PhysicalOperation(
        projects,
        filters,
        l @ LogicalRelation(relation: HadoopFsRelation, _, table, _)) =>
      val fsRelation = if (relation.sparkSession.sparkContext.isStopped) {
        relation.copy()(SparkSession.getDefaultSession.get)
      } else {
        relation
      }
