 ...
 */
abstract class AbstractXSQLParser extends ParserInterface with Logging {
  ...
  protected def astBuilder: XSQLAstBuilder
 ...
 */
class CatalystSqlParser(conf: SQLConf) extends AbstractXSQLParser {
  val astBuilder = new XSQLAstBuilder(conf)
}

/** For test-only. */
object CatalystSqlParser extends AbstractXSQLParser {
  val astBuilder = new XSQLAstBuilder(new SQLConf())
}
