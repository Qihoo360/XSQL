 ...
 */
case class XSQLInsertIntoHiveTable(
    table: CatalogTable,
  ...
  private def processInsert(
    ...
    val sessionCatalog = sparkSession.sessionState.catalog.asInstanceOf[XSQLSessionCatalog]
    ...
    if (numDynamicPartitions > 0) {
      ...
      if (numStaticPartitions == 0 &&
          hadoopConf
            .get("hive.exec.dynamic.partition.mode", "strict")
    ...
    table.bucketSpec match {
      case Some(bucketSpec) =>
        ...
        if (hadoopConf.get(enforceBucketingConfig, "true").toBoolean ||
          ...
          logWarning(
            message + s" Inserting data anyways since both $enforceBucketingConfig and " +
    ...
    val partitionAttributes = partitionColumnNames.takeRight(numDynamicPartitions).map { name =>
      query
        .resolve(name :: Nil, sparkSession.sessionState.analyzer.resolver)
        .getOrElse {
    ...
    if (partition.nonEmpty) {
      if (numDynamicPartitions > 0) {
        sessionCatalog.setWorkingDataSource(table.dataSource) {
