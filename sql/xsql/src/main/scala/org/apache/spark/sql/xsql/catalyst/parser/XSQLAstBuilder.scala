...
/**
 * The XSQLAstBuilder converts an ANTLR4 ParseTree into a catalyst Expression, LogicalPlan or
 * TableIdentifier.
 */
class XSQLAstBuilder(conf: SQLConf) extends SqlBaseBaseVisitor[AnyRef] with Logging {
  ...
  override def visitSingleTableIdentifier(ctx: SingleTableIdentifierContext): TableIdentifier =
    withOrigin(ctx) {
   ...
   */
  override def visitMultiInsertQuery(ctx: MultiInsertQueryContext): LogicalPlan =
    withOrigin(ctx) {
   ...
   */
  override def visitSingleInsertQuery(ctx: SingleInsertQueryContext): LogicalPlan =
    withOrigin(ctx) {
      plan(ctx.queryTerm).
      // Add organization statements.
   ...
   */
  private def withInsertInto(ctx: InsertIntoContext, query: LogicalPlan): LogicalPlan =
    withOrigin(ctx) {
      ctx match {
   ...
   */
  override def visitInsertIntoTable(ctx: InsertIntoTableContext): InsertTableParams =
    withOrigin(ctx) {
   ...
   */
  override def visitInsertOverwriteTable(ctx: InsertOverwriteTableContext): InsertTableParams =
    withOrigin(ctx) {
   ...
   */
  override def visitInsertOverwriteDir(ctx: InsertOverwriteDirContext): InsertDirParams =
    withOrigin(ctx) {
   ...
   */
  override def visitPartitionSpec(ctx: PartitionSpecContext): Map[String, Option[String]] =
    withOrigin(ctx) {
   ...
   */
  protected def visitNonOptionalPartitionSpec(ctx: PartitionSpecContext): Map[String, String] =
    withOrigin(ctx) {
   ...
   */
  private def withWindows(ctx: WindowsContext, query: LogicalPlan): LogicalPlan =
    withOrigin(ctx) {
      // Collect all window specifications defined in the WINDOW clause.
      val baseWindowMap = ctx.namedWindow.asScala.map { wCtx =>
   ...
   */
  private def withHints(ctx: HintContext, query: LogicalPlan): LogicalPlan = withOrigin(ctx) {
    var plan = query
    ctx.hintStatements.asScala.reverse.foreach {
      case stmt =>
        plan =
          UnresolvedHint(stmt.hintName.getText, stmt.parameters.asScala.map(expression), plan)
    }
    plan
  }
   ...
   */
  private def withGenerate(query: LogicalPlan, ctx: LateralViewContext): LogicalPlan =
    withOrigin(ctx) {
  ...
  override def visitDbIdentifier(ctx: DbIdentifierContext): (Option[String], Option[String]) =
    withOrigin(ctx) {
      var ds: Option[String] = None
      var db: Option[String] = None
      if (ctx != null) {
        ds = Option(ctx.ds).map(_.getText)
        db = Option(ctx.db).map(_.getText)
      }
      (ds, db)
    }
   ...
   * ******************************************************************************************** */
  /**
   * Create a [[TableIdentifier]] from a 'tableName'
   *  or 'databaseName'.'tableName'
  ...
  override def visitTableIdentifier(ctx: TableIdentifierContext): TableIdentifier =
    withOrigin(ctx) {
      val ds = Option(ctx.ds).map(_.getText)
      val db = Option(ctx.db).map(_.getText)
      TableIdentifier(ctx.table.getText, db, ds)
    }

  /**
   * Create a [[FunctionIdentifier]] from a 'functionName' or 'databaseName'.'functionName' pattern.
   */
  override def visitFunctionIdentifier(ctx: FunctionIdentifierContext): FunctionIdentifier =
    withOrigin(ctx) {
      val ds = Option(ctx.ds).map(_.getText)
      val db = Option(ctx.db).map(_.getText)
      FunctionIdentifier(ctx.function.getText, db, ds)
    }
  ...
  override def visitFunctionCall(ctx: FunctionCallContext): Expression = withOrigin(ctx) {
    ...
    val arguments = ctx.argument.asScala.map(expression) match {
      case Seq(UnresolvedStar(None)) if name.toLowerCase(Locale.ROOT) == "count" && !isDistinct =>
  ...
  protected def visitFunctionName(ctx: QualifiedNameContext): FunctionIdentifier = {
    ctx.identifier().asScala.map(_.getText) match {
      case Seq(ds, db, fn) => FunctionIdentifier(fn, Option(db), Option(ds))
  ...
  override def visitWindowDef(ctx: WindowDefContext): WindowSpecDefinition = withOrigin(ctx) {
    ...
    WindowSpecDefinition(partition, order, frameSpecOption.getOrElse(UnspecifiedFrame))
  }
  ...
  private def visitSparkDataType(ctx: DataTypeContext): DataType = {
    val dt = MongoStringType.replaceCharType(
      ElasticSearchStringType.replaceCharType(HiveStringType.replaceCharType(typedVisit(ctx))))
    dt
  }
  ...
  override def visitPrimitiveDataType(ctx: PrimitiveDataTypeContext): DataType = withOrigin(ctx) {
    ...
    (dataType, ctx.INTEGER_VALUE().asScala.toList) match {
      ...
      case (dt, params) =>
        Option(ExternalTypeParser.visit(ctx)).getOrElse {
  ...
  override def visitColTypeList(ctx: ColTypeListContext): Seq[StructField] = withOrigin(ctx) {
    val cols = ctx.colType().asScala
    val normalCols = cols.filter(_.dataType() != null).map(visitColType)
    val pkName = cols.filter(col => col.PRIMARY() != null && col.identifierList != null)
    val keyCols = normalCols.filter(_.metadata.contains(PRIMARY_KEY))
    if (pkName.size > 1 || keyCols.size > 1 || (pkName.size > 0 && keyCols.size > 0)) {
      throw new SparkException(s"Not allowed multi primary key block!")
    } else if (pkName.size == 1) {
      val pkColnames = visitIdentifierList(pkName.head.identifierList())
      normalCols.map(field => {
        if (pkColnames.contains(field.name)) {
          val builder = new MetadataBuilder
          val newMeta = builder
            .withMetadata(field.metadata)
            .putBoolean(PRIMARY_KEY, true)
            .build()
  ...
  override def visitColType(ctx: ColTypeContext): StructField = withOrigin(ctx) {
    ...
    if (commentString != null) {
      builder.putString("comment", string(commentString))
    }

    var unReplaced = true
    val rawDataType = typedVisit[DataType](ctx.dataType)
    // Add Hive type string to metadata.
    var cleanedDataType = HiveStringType.replaceCharType(rawDataType)
    if (rawDataType != cleanedDataType) {
      builder.putString(HIVE_TYPE_STRING, rawDataType.catalogString)
      unReplaced = false
    }
    if (unReplaced) {
      // Add Elastic Search type string to metadata.
      cleanedDataType = ElasticSearchStringType.replaceCharType(cleanedDataType)
      if (rawDataType != cleanedDataType) {
        builder.putString(ELASTIC_SEARCH_TYPE_STRING, rawDataType.catalogString)
        unReplaced = false
      }
    }
    if (unReplaced) {
      // Add MongoDB type string to metadata.
      cleanedDataType = MongoStringType.replaceCharType(cleanedDataType)
      if (unReplaced && rawDataType != cleanedDataType) {
        builder.putString(MONGO_TYPE_STRING, rawDataType.catalogString)
        unReplaced = false
      }
    }
    var nullable = true
    ...
    StructField(identifier.getText, cleanedDataType, nullable = nullable, builder.build())
  }
