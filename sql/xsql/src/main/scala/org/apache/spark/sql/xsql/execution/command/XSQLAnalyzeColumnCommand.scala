 ...
 * query optimizations.
 */
case class XSQLAnalyzeColumnCommand(tableIdent: TableIdentifier, columnNames: Seq[String])
  ...
  private def computeColumnStats(
    ...
    val statsRow =
      new QueryExecution(sparkSession, Aggregate(Nil, namedExpressions, relation)).executedPlan
        .executeTake(1)
        .head

    val rowCount = statsRow.getLong(0)
    val columnStats = attributesToAnalyze.zipWithIndex.map {
      case (attr, i) =>
        // according to `statExprs`, the stats struct always have 7 fields.
        (
          attr.name,
          rowToColumnStat(
            statsRow.getStruct(i + 1, 7),
            attr,
            rowCount,
  ...
  private def computePercentiles(
    ...
    if (attrsToGenHistogram.nonEmpty) {
      val percentiles = (0 to conf.histogramNumBins)
        .map(i => i.toDouble / conf.histogramNumBins)
        .toArray
      ...
      val percentilesRow =
        new QueryExecution(sparkSession, Aggregate(Nil, namedExprs, relation)).executedPlan
          .executeTake(1)
          .head
      attrsToGenHistogram.zipWithIndex.foreach {
        case (attr, i) =>
  ...
  private def statExprs(
    ...
    col.dataType match {
      ...
      case _ =>
        throw new AnalysisException(
          "Analyzing column statistics is not supported for column " +
  ...
  private def rowToColumnStat(
    ...
    if (row.isNullAt(6) || cs.nullCount.isEmpty) {
      ...
      val bins = ndvs.zipWithIndex.map {
        case (ndv, i) =>
