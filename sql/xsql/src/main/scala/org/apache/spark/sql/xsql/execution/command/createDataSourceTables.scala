...
import org.apache.spark.sql.xsql.{DataSourceManager, XSQLSessionCatalog}
 ...
 * }}}
 */
case class XSQLCreateDataSourceTableCommand(table: CatalogTable, ignoreIfExists: Boolean)
  ...
  override def run(sparkSession: SparkSession): Seq[Row] = {
    ...
    val catalog = sessionState.catalog.asInstanceOf[XSQLSessionCatalog]
    val tableIdentifier = catalog.getUsedTableIdentifier(table.identifier)
    ...
    val defaultOptions = catalog.getDefaultOptions(tableWithDefaultOptions)
    ...
    val dataSource: BaseRelation =
      DataSource(
        ...
        options = table.storage.properties ++ pathOption ++ defaultOptions,
    ...
    sessionState.catalog.createTable(newTable, ignoreIfExists = ignoreIfExists)
 ...
 * }}}
 */
case class XSQLCreateDataSourceTableAsSelectCommand(
    table: CatalogTable,
    mode: SaveMode,
    query: LogicalPlan,
  ...
  override def run(sparkSession: SparkSession, child: SparkPlan): Seq[Row] = {
    ...
    val sessionState = sparkSession.sessionState
    val catalog = sessionState.catalog.asInstanceOf[XSQLSessionCatalog]
    val tableIdentWithDB = catalog.getUsedTableIdentifier(table.identifier)

    val defaultOptions = catalog.getDefaultOptions(table)
    val realTableType =
      defaultOptions.getOrElse(DataSourceManager.TABLETYPE, table.tableType.name)
    ...
    if (sessionState.catalog.tableExists(tableIdentWithDB)) {
      ...
      if (mode == SaveMode.ErrorIfExists) {
        throw new AnalysisException(
          s"Table $tableName already exists. You need to drop it first.")
      }
      ...
      val tableLocation = if (realTableType.equals(CatalogTableType.MANAGED.name)) {
  ...
  private def saveDataIntoTable(
      ...
      mode: SaveMode,
      tableExists: Boolean,
      defaultOptions: Map[String, String]): BaseRelation = {
