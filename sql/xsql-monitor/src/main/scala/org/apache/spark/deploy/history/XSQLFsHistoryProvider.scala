 ...
 */
private[history] class XSQLFsHistoryProvider(conf: SparkConf, clock: Clock)
  ...
  import config._
  import XSQLFsHistoryProvider._

  private val scanStartTime = conf
    .getOption("spark.history.fs.scan.start")
    .map(DateUtils.parseDate(_, "yyyy-MM-dd HH:mm:ss", "yyyy-MM-dd").getTime)
  private val scanEndTime = conf
    .getOption("spark.history.fs.scan.end")
    .map(DateUtils.parseDate(_, "yyyy-MM-dd HH:mm:ss", "yyyy-MM-dd").getTime)
  ...
  private val eventsFilter: ReplayEventsFilter = { eventString =>
    ...
    eventString.startsWith(EXEC_ADD_EVENT_PREFIX) ||
    eventString.startsWith(EXEC_REMOVE_EVENT_PREFIX) ||
    eventString.startsWith(SQL_EVENT_PREFIX)
  ...
  private val NUM_PROCESSING_THREADS = conf.getInt(
    SPARK_HISTORY_FS_NUM_REPLAY_THREADS,
  ...
  logInfo(
    s"History server ui acls " + (if (HISTORY_UI_ACLS_ENABLE) "enabled" else "disabled") +
    ...
    try {
      ...
      val updated = Option(fs.listStatus(new Path(logDir)))
        ...
        .filter { entry =>
          ...
          scanStartTime.map(_ < entry.getModificationTime).getOrElse(true) &&
          scanEndTime.map(_ > entry.getModificationTime).getOrElse(true)
        }
        .filter { entry =>
          try {
            ...
            case _: NoSuchElementException =>
              ...
              listing.write(
                LogInfo(entry.getPath().toString(), newLastScanTime, None, None, entry.getLen()))
              entry.getLen() > 0
          }
        }
        .sortWith {
...
//            override def run(): Unit = mergeApplicationListing(entry, newLastScanTime, true)
            override def run(): Unit = {
              rebuildAppStore(new InMemoryStore(), entry, newLastScanTime)
            }
          }, Unit)
  ...
  override def writeEventLogs(
    ...
    try {
      ...
      attemptId
        .map { id =>
          app.attempts.filter(_.info.attemptId == Some(id))
        }
   ...
   */
  private def rebuildAppStore(store: KVStore, eventLog: FileStatus, lastUpdated: Long): Unit = {
  ...
  private def addListing(app: ApplicationInfoWrapper): Unit = listing.synchronized {
    ...
    val newAppInfo = new ApplicationInfoWrapper(app.info, attempts.sortWith(compareAttemptInfo))
    listing.write(newAppInfo)
  }
  ...
  private def loadDiskStore(
    ...
    val isCompressed = EventLoggingListener
      .codecName(status.getPath())
      .flatMap { name =>
  ...
  private[history] def getAttempt(
      appId: String,
      attemptId: Option[String]): AttemptInfoWrapper = {
    load(appId).attempts
...
private[history] object XSQLFsHistoryProvider {
  ...
  private val EXEC_ADD_EVENT_PREFIX = "{\"Event\":\"SparkListenerExecutorAdded\""

  private val EXEC_REMOVE_EVENT_PREFIX = "{\"Event\":\"SparkListenerExecutorRemoved\""
  ...
  private val SQL_EVENT_PREFIX = "{\"Event\":\"org.apache.spark.sql.execution.ui"
