...
sealed trait IdentifierWithDatabase {
  ...
  def quotedString: String = {
    ...
    val replacedDs = dataSource.map(quoteIdentifier(_))

    if (replacedDs.isDefined && replacedDb.isDefined) {
      s"`${replacedDs.get}`.`${replacedDb.get}`.`$replacedId`"
  ...
  def unquotedString: String = {
    if (dataSource.isDefined && database.isDefined) {
      s"${dataSource.get}.${database.get}.$identifier"
 ...
 */
case class AliasIdentifier(
    identifier: String,
    database: Option[String] = None,
    dataSource: Option[String] = None)
  extends IdentifierWithDatabase {

  def this(identifier: String) = this(identifier, None, None)
}
...
case class QualifiedTableName(database: String, name: String, dataSource: Option[String] = None) {
  override def toString: String = s"${dataSource.get}.$database.$name"
  def this(database: String, name: String) = this(database, name, None)
}
