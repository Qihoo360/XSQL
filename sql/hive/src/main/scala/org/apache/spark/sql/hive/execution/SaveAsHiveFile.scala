...
import org.apache.spark.sql.hive.HiveUtils.HIVE_METASTORE_VERSION
import org.apache.spark.sql.hive.client.{HiveVersion, IsolatedClientLoader}

// Base trait from which all hive insert statement physical execution extends.
private[sql] trait SaveAsHiveFile extends DataWritingCommand {
  ...
  protected def getExternalTmpPath(
    ...
    val hiveVersion = IsolatedClientLoader.hiveVersion(conf.getConf(HIVE_METASTORE_VERSION))
