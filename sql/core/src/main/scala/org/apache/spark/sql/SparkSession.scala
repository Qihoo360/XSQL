...
class SparkSession private(
  ...
  private[sql] def restartSparkContext(restartConf: Map[String, String]): SparkSession = {
    sparkContext.stop()
    val sparkConf = new SparkConf()
    restartConf.foreach { case (k, v) => sparkConf.set(k, v) }
    val sc = SparkContext.getOrCreate(sparkConf)
    val result = new SparkSession(
      sc,
      Some(sharedState.restartSparkContext(sc)),
      Some(sessionState),
      extensions)
    result.sessionState // force copy of SessionState
    SparkSession.setDefaultSession(result)
...
object SparkSession extends Logging {
  ...
  class Builder extends Logging {
    ...
    /**
     * Enables XSQL support, including connectivity to various datasource's persistent metastore,
     * support for XSQL serdes, and XSQL user-defined functions.
     *
     * @since 2.3.1
     */
    def enableXSQLSupport(): Builder = synchronized {
      if (xsqlClassesArePresent) {
        config(CATALOG_IMPLEMENTATION.key, "xsql")
      } else {
        throw new IllegalArgumentException(
          "Unable to instantiate SparkSession with XSQL support because " +
              "XSQL classes are not found.")
      }
    }
  ...
  private val XSQL_SESSION_STATE_BUILDER_CLASS_NAME =
    "org.apache.spark.sql.xsql.XSQLSessionStateBuilder"

  private def sessionStateClassName(conf: SparkConf): String = {
    conf.get(CATALOG_IMPLEMENTATION) match {
      case "hive" => HIVE_SESSION_STATE_BUILDER_CLASS_NAME
      case "in-memory" => classOf[SessionStateBuilder].getCanonicalName
      case "xsql" => XSQL_SESSION_STATE_BUILDER_CLASS_NAME
    }
  }
   ...
   * @return true if Hive classes can be loaded, otherwise false.
   */
  private[spark] def xsqlClassesArePresent: Boolean = {
    try {
      Utils.classForName(XSQL_SESSION_STATE_BUILDER_CLASS_NAME)
      Utils.classForName("org.apache.hadoop.hive.conf.HiveConf")
      // TODO add other datasource's must class.
      true
    } catch {
      case _: ClassNotFoundException | _: NoClassDefFoundError => false
    }
