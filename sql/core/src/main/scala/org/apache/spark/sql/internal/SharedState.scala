...
private[sql] class SharedState(val sparkContext: SparkContext) extends Logging {
   ...
   */
  var statusStore: SQLAppStatusStore = {
  ...
  lazy val externalCatalog: ExternalCatalogWithListener = {
    ...
    if (!sparkContext.conf.get(CATALOG_IMPLEMENTATION).equals("xsql")) {
      ...
      if (!externalCatalog.databaseExists(SessionCatalog.DEFAULT_DATABASE)) {
        // There may be another Spark application creating default database at the same time, here
        // we set `ignoreIfExists = true` to avoid `DatabaseAlreadyExists` exception.
  ...
  def restartSparkContext(sparkContext: SparkContext): SharedState = {
...
object SharedState extends Logging {
  ...
  private val XSQL_EXTERNAL_CATALOG_CLASS_NAME = "org.apache.spark.sql.xsql.XSQLExternalCatalog"

  private def externalCatalogClassName(conf: SparkConf): String = {
    conf.get(CATALOG_IMPLEMENTATION) match {
      case "hive" => HIVE_EXTERNAL_CATALOG_CLASS_NAME
      case "in-memory" => classOf[InMemoryCatalog].getCanonicalName
      case "xsql" => XSQL_EXTERNAL_CATALOG_CLASS_NAME
    }
  }
  ...
  private def reflect[T, Arg1 <: AnyRef, Arg2 <: AnyRef](
    ...
    try {
      ...
      case NonFatal(e) =>
        val directException = if (e.isInstanceOf[InvocationTargetException] &&
            e.getCause() != null) {
          e.getCause()
        } else {
          e
        }
        throw new IllegalArgumentException(s"Error while instantiating '$className':",
          directException)
    }
  }
}
