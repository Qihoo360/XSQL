...
case class ExecutedCommandExec(cmd: RunnableCommand) extends LeafExecNode {
  ...
  protected[sql] lazy val unsafeResult: Seq[InternalRow] = {
    val proj = UnsafeProjection.create(schema)
    proj.initialize(0)
    sideEffectResult.map { r =>
      proj(r).copy()
    }
  }
  ...
  override def executeCollect(): Array[InternalRow] = unsafeResult.toArray

  override def executeCollectPublic(): Array[Row] = cmd.run(sqlContext.sparkSession).toArray
  ...
  protected override def doExecute(): RDD[InternalRow] = {
    sqlContext.sparkContext.parallelize(unsafeResult, 1)
  }
}
