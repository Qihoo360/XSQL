...
if [ "$NAME" == "none" ]; then
#  NAME=hadoop-$SPARK_HADOOP_VERSION
...
BUILD_COMMAND=("$MVN" -T 1C package -DskipTests $@ -am -pl assembly,sql/xsql-hbase,sql/xsql-druid)
...
# Only create the hbase directory if the hbase artifacts were built.
if [ -d "$SPARK_HOME"/sql/xsql-hbase/target/scala*/jars ]; then
  mkdir "$DISTDIR/hbase"
  for filename in `ls "$SPARK_HOME"/sql/xsql-hbase/target/scala*/jars`;
  do
    if [ ! -f "$SPARK_HOME"/assembly/target/scala*/jars/"$filename" ]; then
        cp "$SPARK_HOME"/sql/xsql-hbase/target/scala*/jars/"$filename" "$DISTDIR/hbase"
    fi
    done
fi

# Only create the druid directory if the hbase artifacts were built.
if [ -d "$SPARK_HOME"/sql/xsql-druid/target/scala*/jars ]; then
  mkdir "$DISTDIR/druid"
  for filename in `ls "$SPARK_HOME"/sql/xsql-druid/target/scala*/jars`;
  do
    if [ ! -f "$SPARK_HOME"/assembly/target/scala*/jars/"$filename" ]; then
        cp "$SPARK_HOME"/sql/xsql-druid/target/scala*/jars/"$filename" "$DISTDIR/druid"
    fi
    done
fi
...
if [ "$MAKE_TGZ" == "true" ]; then
  TARDIR_NAME=xsql-$VERSION-bin-$NAME
  TARDIR="$SPARK_HOME/$TARDIR_NAME"
  rm -rf "$TARDIR"
  cp -r "$DISTDIR" "$TARDIR"
  tar czf "xsql-$VERSION-bin-$NAME.tgz" -C "$SPARK_HOME" "$TARDIR_NAME"
  rm -rf "$TARDIR"
fi
